...
%   trend(trend==0)=NaN;
  ...
  if g==1
	...
	for i=2:size(cycle,2)
	  ...
	  figure;plot(cycle(:,i));title(sprintf('Componente cíclico %g',Pcycle(i-1)));
	end 
	figure;plot(cycle(:,1));title('Componente cíclico total');
      else 
	figure;plot(cycle);title('Componente cíclico');
      end
    end
